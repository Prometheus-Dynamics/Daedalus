...
impl CppManifestPlugin {
    ...
    fn install_inner(&self, registry: &mut PluginRegistry) -> Result<(), CppManifestError> {
        ...
        for node in &self.manifest.nodes {
            let desc = manifest_node_to_descriptor(node).map_err(|e| {
                CppManifestError::Registry(format!("invalid descriptor for {}: {}", node.id, e))
            })?;
            registry
                .registry
                .register_node(desc)
...
impl Plugin for CppManifestPlugin {
    ...
    fn install(&self, registry: &mut PluginRegistry) -> Result<(), &'static str> {
        self.install_inner(registry)
            .map_err(|_| "cpp manifest install failed")
    }
}
